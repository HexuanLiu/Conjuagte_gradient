function [resid, resest, Tk, Zk, fknorms, inprods, xkdiff, errA, errAest] = cgcg(A, b, x0, itmax, flag, x_true)
...
%      Check to see if xk = x0 + Zk*(Tk\(resid(1)*e1)).
  ...
  wk = A*rk; bknum = rk'*rk; bk = bknum/bkden; bkden = bknum;
  pk = rk + bk*pk;
  sk = wk + bk*sk;
  akm1 = ak;
  akden = rk'*wk; ak = bknum/(akden - (bk/ak)*bknum);
