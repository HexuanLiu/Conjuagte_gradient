function [resid, resest, Tk, Zk, fknorms, inprods, xkdiff, errA, errAest] = hscg(A, b, x0, itmax, flag, x_true)
%
%  Hestenes-Stiefel CG
...
for k=1:itmax,
  Apk = A*pk;
  akden = Apk'*pk; ak = bkden/akden;
  if k==1,
    Tk(k,k) = 1/ak;
  else
    Tk(k,k) = 1/ak + bkm1/akm1;
  end;
  xk = xk + ak*pk;
%      Check to see if xk = x0 + Zk*(Tk\(resid(1)*e1)).
  ...
  bknum = rk'*rk; bk = bknum/bkden; bkden = bknum;
  pk = rk + bk*pk;
  akm1 = ak; bkm1 = bk;
end;
